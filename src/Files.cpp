...
PhysicalFile::PhysicalFile(const wstring& filename) : File(filename)
{
	offset = 0;
	handle = new Handle;
...
SubFile::SubFile(File* file, const std::wstring& filename, unsigned long start, unsigned long size) : File(file->getName() + TEXT("|") + filename)
{
	this->file   = file;
	this->offset = 0;
	this->start  = start;
